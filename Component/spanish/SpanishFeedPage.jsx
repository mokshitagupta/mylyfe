...
function CommentSection(props) {
  ...
  return (
    <div className={`cs-wrapper ${props.state ? 'open' : ''}`}>
      {!(comments.length == 0) ? comments.map((comment) => <Comment comment={comment}/>) : <p> Sé el primer comentarista ;) </p>}
      ...
      <div className = "feed-box" ref={goalRef} onClick={(event) => handleFocus(event)}>
            ...
            <div className = "footbox">
              ...
              <div className="cin-wrapper">
                <input className="comment-bar" onClick={() => toggleComment(true)}placeholder="Escribir un comentario . . ." type="text" onKeyDown={
                  (event) => {
                    if(event.key === 'Enter'){
...
class SpanishFeedPage extends React.Component {
  constructor(props) {
    super(props);
 ...
 async componentDidMount(){
  ...
  checkForErrors() {
    if(this.state.followList.length === 0){
      ...
      this.setState({error: "¡No estas siguiendo a nadie!"});
    }
    else{
      ...
      if (result[0].length !== 0) {
        ...
        for(let i = 0; i < postsLoaded; i++){
          ...
          if (result[0][i].attributes.status === "completed"){
            newTitle = "Completó un nuevo meta: " + result[0][i].attributes.title;
          }
          else{
            newTitle = "Comenzó una nueva meta: " + result[0][i].attributes.title;
          }
          let pic = img;
    ...
    .then((result) => {
      if (result[0].length !== 0) {
        ...
        for(let i = 0; i < postsLoaded; i++){
          let newTitle = "";
          if (result[0][i].attributes.status === "completed"){
            newTitle = "Completó un nuevo objetivo: " + result[0][i].attributes.title;
          }
          else{
            newTitle = "Comenzó una nueva meta: " + result[0][i].attributes.title;
          }
  ...
  render(){
    return (
      <div className = "bodyfeed">
        <p>Esto es lo que han estado haciendo tus seguidores...</p>
...
export default SpanishFeedPage;