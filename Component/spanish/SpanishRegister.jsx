...
class SpanishRegister extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      email: '', 
  ...
  validate = (email, data, confirm) => {
    ...
    if (!emailRegex.test(email)){
      ...
      this.setState({error: "¡Por favor introduzca una dirección de correo electrónico válida!"});
      return false;
    }
    ...
    if (ustring === data){
      this.setState({isError: true});
      this.setState({error: "Su contraseña debe tener una letra mayúscula."});
      return false;
    }
  ...
  async passwordProcessing(){
    ...
    if (result.status == 500){
      ...
      this.setState({isError: true});
      this.setState({error: "¡La cuenta ya existe! Por favor Iniciar sesión"});
      return;
    }
  ...
  return ( <div className = 'backgroundContainer'>  
      ...
      <form className = "regbox">
          ...
          <span className='placeholder'>{this.state.isActive ? "" : "Ingrese correo electrónico"}</span>
          ...
          <span className='passwordholder'>Contraseña</span> 
          ...
          <span className='confirmpasswordholder'>Confirmar Contraseña</span>
          <p className='instructions'> La contraseña debe tener entre 8 y 20 caracteres y tener al menos una letra mayúscula y minúscula y un número.</p>
          <div className = "forgotPass2"><a href = "/hci/teams/aquafit/login" id='forgotPassword'>¿Ya tienes una cuenta? ¡Haga clic aquí!</a></div>
      </form>
      
      <button className='regbtn' onClick={this.passwordProcessing}>Registro</button>
...
export default SpanishRegister;
