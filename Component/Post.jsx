...
export default class Post extends React.Component {
  ...
  tagPost(tag, thisPostID){
      ...
      this.props.post.reactions.forEach(reaction => {
        if (reaction.reactorID === localStorage.getItem("user")){
     ...
     fetch(process.env.REACT_APP_API_PATH+"/post-reactions", {
      ...
      body: JSON.stringify({
        reactorID: localStorage.getItem("user"),
        postID: thisPostID,
        name: "like"
      })
      })
    ...
    if (this.state.showTags) {
      ...
      if (this.props.post.reactions.length > 0){
        for (let i = 0; i < this.props.post.reactions.length; i++){
          if (this.props.post.reactions[i].reactorID === localStorage.getItem("user")){ 
  ...
  // author of the post
  showDelete(){
    if (this.props.post.author.id === localStorage.getItem("user")) {
      return(
      <img
        src={helpIcon}
